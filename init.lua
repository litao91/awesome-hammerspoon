...
if spoon.ClipShow then
    ...
    cmodal:bind('', 'M', 'Open in NVim', function()
        spoon.ClipShow:openWithCommand("/Users/taoli/.local/bin/vimr")
        spoon.ClipShow:toggleShow()
        spoon.ModalMgr:deactivate({"clipshowM"})
    end)
...
if spoon.Seal then
  print("loading seal")
  spoon.Seal:loadPlugins({"apps", "screencapture", "safari_bookmarks", "calc", "useractions", "qalc", "youdao", "btabs", "dateformat", "chrome_history", "chrome_bookmarks", "task", "pasteboard" })
  spoon.Seal:start()
...
if spoon.WinWin then
    ...
    if string.len(hsresizeM_keys[2]) > 0 then
        spoon.ModalMgr.supervisor:bind(hsresizeM_keys[1], hsresizeM_keys[2], "Enter resizeM Environment", function()
            hs.alert("Resizing Window Modal entered")
